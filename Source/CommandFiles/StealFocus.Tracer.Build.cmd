...
IF /i %buildChoice% == f (
ECHO.
%windir%\Microsoft.NET\Framework64\v4.0.30319\msbuild.exe ..\Scripts\StealFocus.Tracer.Build.proj /t:CleanSolution /consoleloggerparameters:Verbosity=minimal /fileLogger /fileLoggerParameters:LogFile=CleanSolution.msbuild.log;verbosity=diagnostic
ECHO.
%windir%\Microsoft.NET\Framework64\v4.0.30319\msbuild.exe ..\Scripts\StealFocus.Tracer.Build.proj /p:IsAllConfigurationsBuild=true;IsCommandLineBuild=true;IsDocumentationBuild=false;IsInitialiseEnvironmentBuild=true;IsPackagedBuild=true /consoleloggerparameters:Verbosity=minimal /fileLogger /fileLoggerParameters:LogFile=SolutionBuild.msbuild.log;verbosity=diagnostic
) ELSE (
	IF /i %buildChoice% == q (
	ECHO.
	%windir%\Microsoft.NET\Framework64\v4.0.30319\msbuild.exe ..\Scripts\StealFocus.Tracer.Build.proj /p:IsCommandLineBuild=true /consoleloggerparameters:Verbosity=minimal /fileLogger /fileLoggerParameters:LogFile=SolutionBuild.msbuild.log;verbosity=diagnostic
	) ELSE (
		ECHO.
